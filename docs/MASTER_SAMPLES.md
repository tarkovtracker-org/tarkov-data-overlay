...
```json5
{
  ...
  "task-id-1": {
    ...
    maps: [{ id: "map-id-1", name: "Map Name 1" }], // Optional | Console-only; NOT used by TarkovTracker. TT uses task.map for the map badge, and uses objectives[].maps + task.locations (when present) for map filtering/markers.
